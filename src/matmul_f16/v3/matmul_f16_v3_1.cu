...
__global__ void matmul_f16_v3(const float16_t* A, const float16_t* B, float16_t* C, size_t M, size_t N, size_t K) {
    ...
    for(int split_K = 0; split_K < (K + BK -1) / BK; split_K++){
        ...
        for(int ai = 0; ai < BM * BK; ai += 32 * 4 * 8){//8个half作为一个float4进行搬运
            ...
            s_a[s_a_row * (BK + 8) + s_a_col] =(row + s_a_row < M && split_K * BK + s_a_col < K) ? ((const half*)A)[(row + s_a_row)* K + split_K * BK + s_a_col] : __float2half(0.0f);
        }
        #pragma unroll
        for(int bi =0; bi < BK * BN; bi += 32 * 4){
            int s_b_row = (bi + tid) / BN;
            int s_b_col = (bi + tid) % BN;
            s_b[s_b_row * (BN + 8) + s_b_col] =(split_K * BK + s_b_row < K && col + s_b_col < N)? ((const half*)B)[(split_K * BK + s_b_row) * N + col + s_b_col] : __float2half(0.0f);
        }
