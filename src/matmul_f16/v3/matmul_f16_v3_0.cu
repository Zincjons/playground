...
__global__ void matmul_f16_v3(const float16_t* A, const float16_t* B, float16_t* C, size_t M, size_t N, size_t K) {
    ...
    for(int split_K = 0; split_K < (K + BK -1) / BK; split_K++){
        ...
        //每个线程搬 BM*BK/(32*4)
        #pragma unroll
        for(int ai = 0; ai < BM * BK; ai += 32 * 4){
            int s_a_row = (ai + tid) / BK;
            int s_a_col = (ai + tid) % BK;
            s_a[ai + tid] =(row + s_a_row < M && split_K * BK + s_a_col < K) ? ((const half*)A)[(row + s_a_row)* K + split_K * BK + s_a_col] : __float2half(0.0f);
        }
        #pragma unroll
        for(int bi =0; bi < BK * BN; bi += 32 * 4){
            int s_b_row = (bi + tid) / BN;
            int s_b_col = (bi + tid) % BN;
            s_b[bi + tid] =(split_K * BK + s_b_row < K && col + s_b_col < N)? ((const half*)B)[(split_K * BK + s_b_row) * N + col + s_b_col] : __float2half(0.0f);
        }
        ...
        for(int split_BK = 0; split_BK < 2; split_BK++){//
            ...
            wmma::load_matrix_sync(frag_a0,(const half*)s_a + warp_Row * 32 * BK + split_BK * 16, BK);//取出sa中的列向量的头部分
            wmma::load_matrix_sync(frag_a1,(const half*)s_a + (warp_Row * 32 + 16) * BK + split_BK * 16, BK);
            //把sb搬运到frag_b0,frag_b1
            wmma::load_matrix_sync(frag_b0, (const half*)s_b + (split_BK * 16 * BN) + warp_Col * 32, BN);
            wmma::load_matrix_sync(frag_b1, (const half*)s_b + split_BK * 16 * BN + warp_Col * 32 + 16, BN);
